...
/// Adapted to JavaFX, this generator is capable, in combination with [CircleRipple], of creating an
/// effect that is very close to the one seen in Material 3.
///
/// Extends [RippleGeneratorBase] and thus also implementing [RippleGenerator], with some minor additions.
///
/// The generator is a contained surface, meaning that all the animations/effects are actuated on itself or in contained nodes.
/// Yet, it is not an independent component. A [Region] that will be the generator's `owner` is a must requirement.
/// Not only that, it's important to also notice that the owner must add the generator among its children and properly layout it
/// (mostly resize it, location will usually always be at [0, 0]).
///
...
/// the position will come from a [MouseEvent] occurred on the owner. However, consider this example:
/// you have a button with the generator incorporated, the button can be triggered by pressing the ENTER key too,
/// in such case you probably still want to communicate the interaction to the user through a ripple effect.
/// It's enough to add a handler for the desired event type and generate the ripple at the center of the component.
///
/// Ideally, the generator should be 'de-/activated' through the methods [#enable()] and [#disable()], if disposal
/// is needed instead of just disable, call [#dispose()]. However, nothing prevents you from specifying your own handlers.
///
...
///
///  - It's more efficient because now everything is cached, from the ripple to the clip to all the animations.
///  Everything is re-built only when needed, and this is huge for performance.
///  Everything needed by the generator is kept in a 'wrapping' class that represents its state, [GeneratorState].
///  - It's simpler because many properties and methods have been removed, the generator has not been stripped of its
///  core features, just the ones considered useless
///  - It's a bit more complex because now the effect generation is divided in two phases:
///  there's the generation phase and the release phase. This has been done to closely mimic the behavior of the original
///  Material Design ripple effect, more info here: [#generate(double, double)], [#release()], [CircleRipple];
///  That said, one could also implement a ripple that does everything during just one of the two phases, there are many ways to do it.
///
/// Last but not least about the generation, there are a few conditions that can prevent the generator from 'activating':
///  1) Users can disable the generator in various ways: by disabling it, by setting its visibility to hidden
/// (can be done in CSS too, very convenient), or setting its opacity to 0.0 (also settable in CSS)
///  2) By default the generator will check if the given coordinates are withing the owner's bounds, if that is not the
/// case the effect won't be generated, read [#checkBoundsProperty()] and [#canGenerateAt(double, double)] for more info
///
/// The public API, which defines the common features and behaviors of ripple generators ([RippleGenerator]), only
/// specifies a [#generate(double, double)] method that takes the two necessary coordinates. One small addition in this
/// implementation is an easy way to generate effects from a [MouseEvent], see [#generate(MouseEvent)].
///
/// Typically, ripple effects are just circles that fade in/out and grow, this is the default type used by the generator
/// and specified by [#defaultRippleSupplier()]. Users can define new shapes implementing the base API for ripples,
/// [Ripple], and set it on the generator through [#setRippleSupplier(Supplier)].
///
/// Material Design specs show that the effect also animates the background surface; this is probably for a smoother transition.
/// This feature here is disabled by default and can be enabled/customized as needed with [#animateBackgroundProperty()]
/// and [#backgroundColorProperty()]
///
/// Last but not least, the generator is also clipped to avoid the effect trespassing the generator's bounds (which typically
/// correspond to the ones of the owner). There are three ways to clip the generator by setting the [Supplier] responsible
/// for creating the clip node:
///  1) If you set a `null` supplier, the effect will be unbound, it will be able to trespass the bounds
///  2) If you set the [#noClipProperty()] to `true` (also settable from CSS) the effect will be unbound
///  3) If you set the supplier to return `null` then the clip node will be automatically generated based on the
///  owner's geometry
///  4) If you set the supplier to return a valid node than it will be used as the clip
///
///  See also [#buildClip()]
public class MFXRippleGenerator extends RippleGeneratorBase {
    ...
    /// Uses the [Function] set by [#setMeToPosConverter(Function)] to convert the give [MouseEvent]
    /// and feed it to [#generate(Position)].
    ...
    /// Automatically called when [#mouseModeProperty()] changes to update the way [MouseEvent]s are handled.
    ///
    ///  1) [MouseMode#OFF]: mouseTransparent 'false' and pickOnBounds 'true'
    ///  2) [MouseMode#DONT_PICK_ON_BOUNDS]: mouseTransparent 'false' and pickOnBounds 'false'
    ///  3) [MouseMode#MOUSE_TRANSPARENT]: both mouseTransparent and pickOnBounds 'true'
    ...
    ///  1) An handler for [MouseEvent#MOUSE_PRESSED] events that will trigger [#generate(MouseEvent)]
    ///  2) An handler for [MouseEvent#MOUSE_RELEASED] events that will trigger [#release()]
    ///  3) An handler for [MouseEvent#MOUSE_EXITED] events that also will trigger [#release()];
    ///
    /// Material Design Guidelines show that the ripple effect should disappear if the mouse is still pressed but goes
    /// outside the node
    ///
    ...
    /// Removes and sets to `null` the handlers build and added by [#enable()].
    @Override
    public void disable() {
        if (pressed != null) {
    ...
    /// {@inheritDoc}
    ///
    /// Exits immediately if the generator has been disabled ([#isGeneratorDisabled()]) or if the ripple cannot
    /// be positioned at the given coordinated, [#canGenerateAt(double, double)].
    ///
    /// Sets the ripple state to [RippleState#WAITING_FOR_CLICK], then asks the ripple node to reposition through
    /// [Ripple#position(double, double)] and finally asks the ripple to play the in animations, [Ripple#playIn()].
    @Override
    ...
    /// {@inheritDoc}
    ///
    /// Exits immediately if the current ripple state is [RippleState#INACTIVE].
    ///
    /// Otherwise, calls [Ripple#playOut()] and finally set the ripple state to [RippleState#INACTIVE], thus
    /// preventing further 'releases' from occurring.
    @Override
    public void release() {
    ...
    /// Creates the generator's clip node depending on the set [#getClipSupplier()].
    ///  1) If you set a `null` supplier or [#noClipProperty()] to `true`,
    /// the effect will be unbound, it will be able to trespass the bounds.
    ///  2) If you set the supplier to return `null,` then the clip node will be automatically generated based on the
    /// owner's geometry.
    ///  3) If you set the supplier to return a valid node, then it will be used as the clip.
    ///
    ...
    /// one of them. This detection is done by [StyleUtils#parseCornerRadius(Region)].
    @Override
    ...
    /// {@inheritDoc}
    ///
    /// Can be set from CSS via the property: '-mfx-animate-background";
    @Override
    ...
    /// {@inheritDoc}
    ///
    /// Can be set from CSS via the property: '-mfx-background-color";
    @Override
    ...
    /// {@inheritDoc}
    ///
    /// Can be set from CSS via the property: '-mfx-ripple-color";
    @Override
    ...
    /// Specifies whether the generator should first check the coordinates given to [#generate(double, double)] before
    /// generating the effect. See [#canGenerateAt(double, double)].
    ///
    /// Can be set from CSS via the property: '-mfx-check-bounds".
    ...
    /// Allows specifying the behavior of the generator regarding [MouseEvent]s. It may happen (for rounded components in particular)
    /// that Mouse events are intercepted outside the node and thus triggering pseudo states when not intended.
    ///
    /// By default, it's set to [MouseMode#MOUSE_TRANSPARENT] so that the generator ignores any [MouseEvent].
    ...
    /// for any exception.
    ///
    /// Can be set from CSS via the property: '-mfx-mouse-mode'.
    ...
    /// {@inheritDoc}
    ///
    /// Can be set from CSS via the property: '-mfx-no-clip'.
    ...
    /// @return the current ripple effect state, see [RippleState]
    ...
    /// {@inheritDoc}
    ///
    /// If the generator's state is not null, this is also responsible for calling [GeneratorState#updateClip()].
    @Override
    ...
    /// {@inheritDoc}
    ///
    /// If the generator's state is not null, this is also responsible for calling [GeneratorState#updateRipple()].
    @Override
    ...
    /// @return the [Function] used by the generator to convert a [MouseEvent] to a [Position]
    ...
    /// Sets the [Function] used by the generator to convert a [MouseEvent] to a [Position]
    /// bean, which will be used as the coordinates at which animate the ripple
    ...
    ///  - the current state of the effect, see [RippleState]
    ///  - the generator's sizes (width and height)
    ///  - the generator's clip node
    ///  - the generator's ripple node
    ///
    /// When a new state is created, [#init()] must be called after so that the aforementioned properties can be
    /// initialized. A new state is not always required and mostly happens when the generator's bounds change, since these
    /// values may also influence animations. If properties such as [#getRippleSupplier()] or
    /// [#getClipSupplier()] change, then it's enough to update just what's needed.
    public static class GeneratorState {
        ...
        /// Takes and saves the generator's bounds, calls [#updateClip()] and then [#buildRipple()].
        protected void init() {
        ...
        /// Calls [#buildClip()] to create the generator's clip, then sets it.
        protected void updateClip() {
            clip = generator.buildClip();
            generator.setClip(clip);
        }

        /// Calls [#buildRipple()] to create the ripple node, then initializes it through
        /// [#init()], and finally calls [#setRipple(Ripple)].
        ...
        /// @return the ripple effect state, see [RippleState]
        public RippleState getRippleState() {
            return rippleState;
        }
