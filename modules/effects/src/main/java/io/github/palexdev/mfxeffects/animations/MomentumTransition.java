...
/// - The {@code momentum} is the initial velocity of the animation, the speed at which the target will progress towards
...
/// - The {@code acceleration} is always a negative value since this motion in particular is a UDM (Uniformly Decelerated Motion);
...
/// - The {@code displacement} is the difference between the end value and the start value. In an example, let's say
...
///
/// To clarify things, let's see an example with the aforementioned Rectangle:
/// ```
...
/// to make the transition look smooth. This phenomenon is called 'overshoot' and can be described as follows
/// ```plain
...
///
/// You could track the target in an external variable that is not modified by the transition but from the action that leads
...
public class MomentumTransition extends FluentTransition {
    ...
    ///
    /// The momentum and the deceleration are computed automatically by using [#timeToMomentum(double, double)]
    /// and [#momentumToDeceleration(double, double)].
    ...
    ///
    /// The momentum and the duration of the animation are computed automatically by using [#decelerationToMomentum(double, double)]
    /// and [#momentumToTime(double, double)].
    ///
    /// Please note that the `deceleration` param must be a negative number.
    ...
    ///
    /// Formula: `Math.sqrt(-2 * deceleration * displacement)`
    ...
    /// Runs every frame to call the [#getOnUpdate()] consumer.
    ///
    /// The delta displacement given by the [Consumer] is first multiplied by the direction detected when building
    ...
    ///
    /// The current frame time is not computed by simply calling [#getCurrentTime()], but rather we use the 'frac'
    /// parameter of the [#interpolate(double)] method. This is crucial because [Interpolator]s in JavaFX
    /// directly affect the value of the 'frac' parameter, in other words by getting the current frame time as
    /// `getCycleDuration().toMillis() * frac` we automatically take into account the animation's interpolator.
    ...
    /// @return the action that runs every frame of the animation. The [Consumer]
