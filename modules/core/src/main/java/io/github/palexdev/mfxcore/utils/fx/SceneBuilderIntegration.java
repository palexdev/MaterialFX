...
///
/// For example MaterialFX (was!) uses this to set the buttons text to "Button" only if they are used in SceneBuilder, the caveat
...
///
/// MaterialFX also (was!) does another interesting thing with this, since SceneBuilder doesn't offer any way to add custom themes,
...
/// This uses the [StackWalker] API to detect the caller of constructors/methods, the depth is the estimate number
/// of [StackFrames][StackFrame] to process.
public class SceneBuilderIntegration {
    ...
    /// If [#isInSceneBuilder()] returns true executes the given action.
    ...
    /// The [StackWalker] API used to do so, will detect all the classes involved in the calling.
    ...
    /// Sets the 'search' depth used by the [StackWalker] API.
    public static void setDepth(int depth) {
        SceneBuilderIntegration.depth = depth;
        SceneBuilderIntegration.isDepthInvalid = true;
    }

    /// Creates a new file at the given [Path] if non-existent, then writes the given debug String
    /// in it. The 'truncate' boolean flag specifies whether to append the string or clear the file content first.
    ///
    /// For the path you can also use [#DEFAULT_DEBUG_FILE] which points at the 'user.home' directory, and
