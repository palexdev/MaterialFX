...
public class ColumnIterator<T> implements Iterator<List<T>> {
    ...
    /// {@inheritDoc}
    ///
    /// @return whether the cursor is still lesser than [Grid#getColumnsNum()]
    @Override
    public boolean hasNext() {
        return cursor < grid.getColumnsNum();
    }

    /// @return the column at the current cursor value by using [Grid#getColumn(int)]
    @Override
    public List<T> next() {
    ...
    /// Removes the column at the current cursor value by using [Grid#removeColumn(int)].
    ///
    /// The removal is done only if [#hasNext()] is true.
    @Override
    public void remove() {
