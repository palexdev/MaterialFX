...
import static io.github.palexdev.mfxcore.effects.MFXDepthManager.shadowOf;
...
public class RippleGenerator extends Group {
	...
	public RippleGenerator(Region region, RippleClipTypeBuilder factory) {
		this(region);
	...
	public void createRipple() {
		...
		Shape shape = rippleClipTypeBuilder.build(region);
	...
	public void setRippleClipTypeFactory(RippleClipTypeBuilder rippleClipTypeBuilder) {
		this.rippleClipTypeBuilder = rippleClipTypeBuilder;
	}
	...
	private class Ripple extends Circle {
		...
		private Ripple(double centerX, double centerY) {
			...
			setClip(rippleClipTypeBuilder.build(region));
			buildAnimation();
		}
		...
		private void buildAnimation() {
			...
			parallelTransition.setInterpolator(Interpolators.INTERPOLATOR_V2.toInterpolator());
