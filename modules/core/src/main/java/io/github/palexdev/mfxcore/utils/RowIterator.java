...
public class RowIterator<T> implements Iterator<List<T>> {
    ...
    /// {@inheritDoc}
    ///
    /// @return whether the cursor is still lesser than [Grid#getRowsNum()]
    @Override
    public boolean hasNext() {
        return cursor < grid.getRowsNum();
    }

    /// @return the row at the current cursor value by using [Grid#getRow(int)]
    @Override
    public List<T> next() {
    ...
    /// Removes the row at the current cursor value by using [Grid#removeRow(int)].
    ///
