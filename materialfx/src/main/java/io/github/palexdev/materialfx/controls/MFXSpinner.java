...
public class MFXSpinner<T> extends Control implements Themable {
    ...
    public void defaultIcons() {
        setPrevIconSupplier(() -> {
            MFXIconWrapper icon = new MFXIconWrapper("fas-minus", 16, -1).defaultRippleGeneratorBehavior();
        ...
        setNextIconSupplier(() -> {
            MFXIconWrapper icon = new MFXIconWrapper("fas-plus", 16, -1).defaultRippleGeneratorBehavior();
            icon.addEventHandler(MouseEvent.MOUSE_PRESSED, event -> {
                SpinnerModel<T> model = getSpinnerModel();
